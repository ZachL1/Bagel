...
class ImageTranslationIterableDataset(InterleavedBaseIterableDataset):
    def __init__(
        ...
        """
        Image translation dataset for image translation tasks.
        ...
        while True:
            ...
            for row_idx, (data, base_dir) in enumerate(data_paths_per_worker_, start=row_start_id):
                try:
                    ...
                    raw_image_path = os.path.join(base_dir, data_item['src_img_path'])
                    raw_image = pil_img2rgb(Image.open(raw_image_path))
                    
                    # Load target (edited) image
                    target_image_path = os.path.join(base_dir, data_item['tgt_img_path'])
                    target_image = pil_img2rgb(Image.open(target_image_path))
                    
                    # Build instruction
                    src_lang = language_map[data_item['src_lang']]
                    tgt_lang = language_map[data_item['tgt_lang']]
                    src_text : List[str] = data_item['src_text']
                    tgt_text : List[str] = data_item['tgt_text']
                    src_text = str(src_text)
                    tgt_text = str(tgt_text)
                    instruction = f"Translate the {src_lang} text in the image to {tgt_lang}. The source text: {src_text}. The target text: {tgt_text}."
